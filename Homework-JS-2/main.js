...
var textLines = `Городничий. Я пригласил вас, господа, с тем чтобы сообщить вам пренеприятное известие: к нам едет ревизор.
...
let splitText = textLines.split('\n');
let phrasesWithoutAuthor = [];

for(let i = 0; i < splitText.length;i++) {
	let author = splitText[i].split(".")[0];
	let text = splitText[i].split(".")[1];
	var phrase = {
		index: i+1,
		author,
		text
	}
	phrasesWithoutAuthor.push(phrase);
}


console.log(phrasesWithoutAuthor);

let result = phrasesWithoutAuthor.reduce(function(r, a){
	r[a.author] = r[a.author] || []; 
	r[a.author].push( + a.index + ')' + a.text + '<br>');
	return r;
}, [])

var hello = result.map(function(r) {
	return r;
})
...
for (var property in result) {
  ...
  output += property + ': ' + '<br>' + result[property].join('') + '<br>';
